...
class AnalysisView(BaseView):
    ...
    @staticmethod
    async def analysis_board_from_moves(request: Request, moves_str: Optional[str]) -> Response:
    ...
    @staticmethod
    async def analysis_board_rotate(request: Request, moves_str: Optional[str]) -> Response:
        size = request.headers.get("board-size", 13)

        moves = AnalysisView.get_moves_from_str(moves_str, size=size, rotated=True) if moves_str else []
